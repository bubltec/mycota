...
function buildPrompt(domain: string, evidence?: DomainEvidence): string {
  const parts = [
    `Assess the email domain "${domain}" for someone claiming professional/organizational ` +
      'affiliation while contributing to a moderated database. Classify what kind of organization ' +
      'it likely is, and in your reasoning say whether the evidence below actually supports it being ' +
      "a real, established organization — or whether there's no independent trace of it existing.",
  ];
...
export class BedrockClassifierService {
  ...
  constructor(@Inject(MYCOTA_AUTH_CONFIG) config: MycotaAuthConfig) {
    this.client = new BedrockRuntimeClient({ region: config.awsRegion ?? 'us-east-1' });
    this.modelId = config.bedrockInferenceProfileId ?? DEFAULT_MODEL_ID;
  }
